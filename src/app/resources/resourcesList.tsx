        ...
        if (a.bearer_id === u.bearer_id && a.tag_id === u.tag_id) {
          updatedTagReferences.push({
            ...
            resource: [
              [
                u.bearer_id,
                resources.find((r) => r.id === u.bearer_id)?.title || "",
              ],
            ],
            job: [],
          });
      ...
      >
        ...
        <CardBody onClick={() => setShowResourceDescription(true)}>
          <p className="text-medium">
            {props.resource.description.substring(0, MAX_DESCRIPTION_CHAR)}...
          </p>
        </CardBody>
        <Divider />
        <CardFooter>
        ...
        {showResourceDescription && (
          <ResourceDescription
            resource={props.resource}
            onOpenChange={() => setShowResourceDescription(false)}
          />
        )}
      </Card>
    </>
  ...
  async function handleResourceClick() {
    if (props.resource.internal) {
      await endpoints.resources
        .getInternalResource(props.resource.id)
